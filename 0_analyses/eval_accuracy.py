...
def parse_accuracy(logits, labels):
    """
    logits: [T, V] on some device
    labels: [T] on same device, with -100 masked positions
    Each span is exactly one token, so accuracy is just 0 or 1 per span.
    returns: Tensor of per-span accuracy (on CPU)
    """
    mask = labels != -100
    if not mask.any():
        return torch.empty(0, device="cpu")
    return (logits.argmax(dim=-1)[mask] == labels[mask]).float().cpu()
...
if __name__ == "__main__":
    ...
    dataset_full = load_dataset("marcelbinz/Psych-201-discrete-test")["train"]
    ...
    for study in dataset_full.unique("study"):
        ...
        with torch.inference_mode():
            for participant in tqdm(dataset):
                ...
                participant_acc = parse_accuracy(logits, labels)

                key = participant["participant_reindexed"]
                if torch.is_tensor(key):
                    key = int(key.item())
                accs[key] = participant_acc

                del labels, model_outputs
                torch.cuda.empty_cache()
                gc.collect()

        folder_path = "data_acc/" + study.replace("/", "-")
        Path(folder_path).mkdir(parents=True, exist_ok=True)
        torch.save(accs, folder_path + "/" + args.model.replace("/", "-") + ".pth")
